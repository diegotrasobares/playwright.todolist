...
class ToDoListPage extends BasePage {
    ...
    async navigate(){
        await super.navigate('examples/vanillajs/');
    }
    ...
    async getListLength(){
        ...
        let length= await test.step(`Cheking how many tasks are in the list`,async ()=>{
            let length= await this.toDoList.count()
            return length;
        })
        return length;
    ...
    async getTask(n){
         const task= await this.page.locator(`ul > li:nth-child(${n}) > div > label`)
            return task;
