...
/*
 //TODO
  -> Should be able to add emojis ToDo ✔️
  -> Should be able to see active ToDo tasks on the "Active" tab ✔️
  -> Should be able to see completed ToDo tasks on the "Completed" tab ✔️
  -> Should be able to edit Tasks ✔️
  -> Order the reports step-by-step ✔️
  -> Implement JEST for reports
  -> Push to Github

 */


test.describe("Test Suite for ToDoList",async ()=>{
 let page;

 test.beforeAll(async ({browser})=> {
 page= await browser.newPage();
  ToDoListPage = new ToDoListPage(page);
  await ToDoListPage.navigate("examples/vanillajs/");
 });
 ...
 test.only('Should be able to create task with emojis',async ()=>{
  const length= await ToDoListPage.getListLength();
  await ToDoListPage.createToDo('‎😃');

  test.step('',async()=>{
   await expect(await ToDoListPage.getListLength()).toBeGreaterThan(length)
  })
 })
 ...
 test('Should be able to edit ToDos',async({})=>{
  ...
  const task=  await ToDoListPage.getTask(await ToDoListPage.getListLength());

  await test.step('Double click to edit', async () => {
   await task.dblclick()
  })
  await test.step('Fill the new task name', async () => {
   await page.fill(`.todo-list >li:nth-child(${await ToDoListPage.getListLength()}) .edit`, "Edited task");
   await page.keyboard.press('Enter')
  })
  const editedtask= await ToDoListPage.getTask(await ToDoListPage.getListLength());
...
//Assertions
 ...
 test('Should be able to select ToDo', async ({  }) => {
  ...
  await ToDoListPage.selectToDo(await ToDoListPage.getListLength())
   await test.step('Check the task checkbox',async ()=>{
    await expect(page.locator(`li:nth-child(${await ToDoListPage.getListLength()}) .toggle `)).toBeChecked();
   })
 });
 ...
 test("Should not be able to add blank ToDos",async({})=>{
  ...
  test.step('Check if the blank ToDo was added',async ()=>{
   await expect(await ToDoListPage.getListLength()).not.toBeGreaterThan(length)
  })
 })
 ...
 test("Should be able to change to 'Active' tab and show tasks",async({ })=>{
  ...
  let activeTask=  await ToDoListPage.getTask(await ToDoListPage.getListLength());
  ...
  await test.step('Verify the task',async()=>{
   await expect(await ToDoListPage.getTask(await ToDoListPage.getListLength())).toContainText("Active Task")
  })
  await test.step('Verify that is the "Active" tab', async () => {
   await expect(page).toHaveURL("https://todomvc.com/examples/vanillajs/#/active")
 ...
 test("Should be able to change to 'Completed' tab and show tasks",async({ })=>{
  ...
  const completedTask= await ToDoListPage.getTask(await ToDoListPage.getListLength());
  console.log("The complete task is: "+ await completedTask.textContent())
  ...
  //Assertions
  await test.step('Verify the task',async()=>{
   await expect(await ToDoListPage.getTask(await ToDoListPage.getListLength())).toContainText("Completed Task")
  })
  await test.step('Verify that is the "Completed" tab',async()=>{
   await expect(page).toHaveURL("https://todomvc.com/examples/vanillajs/#/completed",{timeout: 2000})
